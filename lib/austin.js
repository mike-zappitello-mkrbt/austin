...
let Austin = function(configuration) {
    ...
    rtmClient.on(RTM_EVENTS.FILE_SHARED, function handleRtmMessage(message) {
      ...
      let fileBasename = path.basename(filename, fileExtension);
      ...
      let sendDm = function(dmInfo, text) {
        return q.Promise(function (resolve, reject) {
      ...
      // check that we can slice this thing
      if (['thing', 'stl', 'obj'].indexOf(fileExtension) < 0) {
        getDmInfo(userId)
        .then(function(dmInfo) {
          let text = 'file extension \'' + fileExtension + '\' not supported.';
          sendDm(dmInfo, text);
        });
        return;
      }
